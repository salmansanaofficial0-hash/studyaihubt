...
export function StickyNewsletterBar() {
  ...
  const [dismissed, setDismissed] = useState(() => {
    ...
    return (
      ...
      !!localStorage.getItem("studyai_subscribed")
    );
  });
  const { email, setEmail, status, message, handleSubmit } = useNewsletterForm("sticky_bar");
      ...
      <div className="max-w-4xl mx-auto rounded-2xl bg-gradient-brand text-white shadow-2xl px-4 py-3 flex items-center gap-3 flex-wrap">
        {status === "success" || status === "duplicate" ? (
          ...
          <>
            ...
            <form onSubmit={onSubmit} className="flex items-center gap-2 flex-1 min-w-[220px]">
              <input
                type="email"
                required
                value={email}
                ...
                className="flex-1 px-3 py-2 rounded-md bg-white/15 backdrop-blur placeholder:text-white/70 text-white text-sm outline-none focus:ring-2 ring-white min-w-0 disabled:opacity-60"
              />
              <button
                disabled={status === "loading"}
            ...
            {status === "error" && message && (
              <p className="w-full text-xs text-white/90 text-center">{message}</p>
            )}
            <button
              onClick={dismiss}
              aria-label="Dismiss"
